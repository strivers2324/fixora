...
func (r *AuthRepository) GetUserSessionData(ctx context.Context, userID uuid.UUID) (*models.SessionResponse, error) {
	query := `SELECT phone, is_phone_verified FROM users WHERE user_id = $1`
	var data models.SessionResponse
	data.Role = string(models.USER)
...
func (r *AuthRepository) GetServiceProviderSessionData(ctx context.Context, providerID uuid.UUID) (*models.SessionResponse, error) {
	query := `
		SELECT
			sp.phone,
			sp.is_phone_verified,
			COALESCE(p.profession_name, '') as profession
		FROM service_providers sp
		LEFT JOIN professions p ON sp.profession_id = p.id
		WHERE sp.provider_id = $1
	`
	var data models.SessionResponse
	data.Role = string(models.SERVICE_PROVIDER)

	err := r.db.QueryRowContext(ctx, query, providerID.String()).Scan(
		&data.Phone,
		&data.IsPhoneVerified,
		&data.Profession,
	)
