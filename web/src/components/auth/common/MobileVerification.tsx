...
export default function MobileVerification({ type }: MobileVerificationProps) {
  ...
  useEffect(() => {
    if (currentOtpId) fetchOTPStatus(currentOtpId);
      ...
      const fallback = type === "service_provider" ? "/service-provider/registration" : "/user/registration";
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const routePrefix = type === "service_provider" ? "/service-provider" : "/user";
      navigate(`${routePrefix}/verify/otp/${newId}`, { replace: true, state: location.state });
  ...
  const handleUpdatePhone = async (values: z.infer<typeof PhoneSchema>) => {
    ...
    try {
      ...
      setSuccessMessage("Code sent successfully!");
      const routePrefix = type === "service_provider" ? "/service-provider" : "/user";
      navigate(`${routePrefix}/verify/otp/${newId}`, {
        replace: true,
            ...
            <div className="text-center w-full">
              <button
                ...
                className={`text-sm transition-all ${
                  limitError ? "text-gray-400 cursor-not-allowed" : "text-teal-600 hover:text-teal-800 hover:underline"
                }`}
              >
