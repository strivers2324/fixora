...
export default function SpMobileVerification() {
    ...
    function onSubmit(data: z.infer<typeof FormSchema>) {
        if (data.pin === "1234") {
            navigate("/SpInformation")
        } else {
            form.setError("pin", { type: "manual", message: "Wrong OTP. Please try again." })
        }
    }
    ...
    return (
        <div className="min-h-screen w-full flex items-center justify-center px-4 bg-gray-200">
            <div className="w-full md:w-1/2 flex items-center justify-center py-8">
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="w-full max-w-md mx-auto space-y-6 bg-white rounded-xl shadow-2xl p-8">
                            ...
                            render={({ field }) => (
                                <FormItem className="flex flex-col items-center">
                                    ...
                                    <FormDescription className="text-center">
                                        Didn&apos;t get your code? <span className="text-teal-900 cursor-pointer hover:underline">Resend</span>
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )}
                        />
                        <Button type="submit" className="w-full bg-teal-900 text-white hover:bg-teal-700 font-serif text-md">Submit</Button>
                    </form>
                </Form>
            </div>
        </div>
